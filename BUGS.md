...
|---|------|-------|--------------------|--------|------------|
| 19 | 2026-08-11 | C3 (found via `iac/prompts/05-https.md`) | Immediately after the deployed VM got HTTPS (Nginx terminating TLS, still reverse-proxying to Jetty over plain HTTP), `POST /api/auth/login` started answering `403 Invalid CORS request` on every attempt — with the correct credentials, from the app's own login page. | Fixed | Nginx already sent `X-Forwarded-Proto: https` to Jetty, but nothing on the Java side trusted it, so `request.getScheme()` still reported `http`. Spring's `CorsUtils.isCorsRequest` treats a request as cross-origin whenever the browser's `Origin` header disagrees with what the server thinks its own scheme/host/port are — same-origin proxying (`iac/requirements.md` decision #5) is supposed to mean that check never even fires for the deployed frontend, and didn't over plain HTTP, where both sides agreed on `http`. Once the browser's `Origin` was `https://...` and Jetty still said `http`, every request looked cross-origin, and `corsConfigurationSource()`'s allowlist (only the Vite dev origin) rejected all of them — including login, the one unauthenticated write nobody could work around from the UI. Fixed by registering `ForwardedHeaderFilter` ahead of `springSecurityFilterChain` in `SecurityWebApplicationInitializer` (`beforeSpringSecurityFilterChain` + `insertFilters`), so `request.getScheme()` correctly reflects `X-Forwarded-Proto` before Spring Security's CORS check runs — restores the actual same-origin invariant instead of widening the allowlist. `SecurityWebApplicationInitializer.java`. |
| 18 | 2026-08-11 | C9 | `POST /api/tenants/register` answered 500 ("Something went wrong") on the happy path — every single call, not just an edge case. | Fixed | `Tenant.status` was still `@Column(length = 16)`, sized for `ACTIVE`/`SUSPENDED`. `PENDING_VERIFICATION` (C9's new status, added in C9(a)) is 21 characters, and MySQL's response to an over-length `VARCHAR` insert is a hard failure (`MysqlDataTruncation: Data too long for column 'status'`), not a silent trim — so every registration attempt failed at the first `INSERT`. Invisible to `mvn test`: nothing in the existing automated suite ever persists a `PENDING_VERIFICATION` tenant, since `register()` (the only code that produces one) had no automated tests yet at C9(a)/(b)/(c) — this is exactly the class of bug manual testing exists to catch before automating around it. Widened to `VARCHAR(32)`; `schema.sql` and `prompts/database/schema.md` regenerated/updated in the same change. |
