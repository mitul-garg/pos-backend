...
- **`GET /api/orders` and `GET /api/returns` no longer N+1 their line items (peer-review Phase 1):** each row's lazy `lines` used to trigger its own `SELECT` — up to 201 round trips for one page. `OrderDao.findLinesByOrderIds`/`ReturnDao.findLinesByReturnIds` batch every order's/return's lines on the page into one `WHERE ... IN (:ids)` query, grouped by parent id; `OrderService`/`ReturnService`'s `toData` mapper now takes the lines explicitly instead of reading the lazy collection itself. The wire contract is unchanged — every row still carries `items[]` exactly as `requirements.md`'s Order/Return shape (above) already promises — this is a backend-only fix, not a leaner list DTO, since trimming `items[]` from list responses would have been a contract change the frontend never asked for. `GET /{id}` and `PaymentService` still read the lazy collection directly for their one row, which was never the N+1.
