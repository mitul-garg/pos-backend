...
- **Product soft-delete:** Products gained an `isActive` flag (Phase 2) so `DELETE /api/products/{id}` can deactivate rather than remove; a product is treated as active unless `isActive === false`.
