...
Spec/roadmap lives in `requirements.md` (this directory) and `../backend-plan.md` (the build sequence, C1–C9). `prompts/` documents what was built and how; `requirements.md` says what to build and tracks resolved decisions — keep both in sync when a decision is made.
...
Note `backend/` is its own git repo — `../backend-plan.md` lives outside it and isn't captured by a backend commit. `requirements.md` and `BUGS.md` live inside `backend/`, so they are.

When the user reports a bug found while manually testing, log it in `BUGS.md` (this directory, backend-only — frontend bugs go in `frontend/BUGS.md` in that repo) and update its status when fixed — routine is at the top of that file.
